...
pub trait Val: Sized + Ord + Eq + Hash {
  ...
  /// Evaluates a constant to some value given a model.
  fn eval_cst(
    cst: & Cst, off: & Offset2, mdl: & Model, factory: & Factory
  ) -> Result<Self, String> {
    use term::CstMaker ;
    let cst = factory.cst(cst) ;
    Self::eval(& cst, off, mdl, factory)
  }
}
      ...
      if let Some(root) = self.root() {
        ...
        for kid in new.nodes.get_mut(& old_key).unwrap().above().iter() {
          match self.nodes.get_mut(kid) {
            Some(kid) => {
              let was_there = kid.above.remove(& old_key) ;
              debug_assert!(was_there) ;
